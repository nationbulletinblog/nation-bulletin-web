...
async function getSearchResults(query: string, limit: number) {
  const sanityQuery = `*[_type == "post" && (title match $searchQuery || excerpt match $searchQuery || categories[]->title match $searchQuery)] | order(publishedAt desc) [0...${limit}] {
    _id,
    title,
    slug,
    excerpt,
...
export default async function SearchPage({ 
  ...
  const initialPosts = query ? await getSearchResults(query, 12) : [];
        ...
        <div className="border-b-2 border-primary pb-8 mb-12 text-center">
            ...
            {query && (
              <p className="text-sm font-medium text-zinc-500 uppercase tracking-widest">
                 Found matching dispatches in our system
              </p>
            )}
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-16">
          ...
          <div className="lg:w-3/4">
             {initialPosts.length > 0 ? (
                <BlogList initialPosts={initialPosts} initialOffset={0} searchQuery={query} />
             ) : (
